 ...
 * @LastEditTime: 2021-01-04 14:25:45
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void)
{
  ...
  { /* mod: keep the gyro heading integrated all the time, not only while driving straight.
       Every 10 ms like the stock straight-line control: the integrator zeroes any step under
       0.05 deg as noise, so calling it every loop iteration would throw the rotation away. */
    static unsigned long yaw_time = 0;
    if (_millis() - yaw_time >= 10)
    {
      yaw_time = _millis();
      AppMPU6050getdata.MPU6050_dveGetEulerAngles(&Yaw_deg);
    }
  }
  { /* mod: telemetry stream {T_<dist>_<yaw*10>_<L>_<M>_<R>_<ground>} every StreamPeriod_ms */
    if (StreamPeriod_ms > 0 && (_millis() - Stream_time) >= StreamPeriod_ms)
    {
      Stream_time = _millis();
      uint16_t d = 0;
      const unsigned long sample_ms = _millis();
      const int pan_tag = activePan.tag(sample_ms);
      if (!ActiveStream || pan_tag > 0)
      ...
      Serial.print('_'); Serial.print(TrackingData_M);
      Serial.print('_'); Serial.print(TrackingData_R);
      Serial.print('_'); Serial.print(Car_LeaveTheGround ? 1 : 0);   // firmware's flag is true while ON the ground
      Serial.print('_'); Serial.print((int)(VoltageData_V * 1000));   // battery, millivolts
      if (ActiveStream) {
        Serial.print('_'); Serial.print(pan_tag);
        Serial.print('_'); Serial.print(++ActiveSequence);
        Serial.print('_'); Serial.print((uint16_t)sample_ms);
      }
      Serial.print('}');
    }
  }
...
void ApplicationFunctionSet::CMD_MotorControlSpeed_xxx0(void)
{
  ...
  {
    MotorControl = true;
    if ((_millis() - MotorSpeed_time) > (ActiveStream ? 500UL : 1500UL)) // mod: shorter v4 renewal deadline
    {
      ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
      Application_SmartRobotCarxxx0.Functional_Mode = CMD_Programming_mode;
    }
    else if (CMD_is_MotorSpeed_L == 0 && CMD_is_MotorSpeed_R == 0)
    {
      ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
    }
    else
...
void ApplicationFunctionSet::CMD_ServoControl_xxx0(void)
{
  ...
  {
    AppServo.DeviceDriverSet_Servo_controls(/*uint8_t Servo*/ CMD_is_Servo, /*unsigned int Position_angle*/ CMD_is_Servo_angle); // mod: degrees
    Application_SmartRobotCarxxx0.Functional_Mode = CMD_Programming_mode; /*进入编程模式提示符<等待下一组控制命令的到来>*/
  }
}
/*
...
void ApplicationFunctionSet::CMD_UltrasoundModuleStatus_xxx0(uint8_t is_get)
{
  ...
  {
    char toString[10];
    itoa(UltrasoundData_cm, toString, 10); // mod: same decimal output without printf's flash overhead
#if _is_print
  ...
  if (c == '}') //数据帧尾部校验
  {
    ...
    else if (!error) //检查反序列化是否成功
    {
      ...
      case 4:                                                                   /*<命令：N 4> */
        ...
        MotorSpeed_time = _millis(); // mod: deadman timestamp
      ...
      case 24: /* mod: gyro heading, tenths of a degree, positive = left */
        Serial.print('{'); Serial.print(CommandSerialNumber); Serial.print('_');
        Serial.print((int)(Yaw_deg * 10)); Serial.print('}');
        break;
      case 26: /* mod: gyro zero recalibration, car must be still; replies {H_ok} when done */
        AppMPU6050getdata.MPU6050_calibration();
        Yaw_deg = 0;
#if _is_print
        Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
        break;
      case 27: /* mod: battery voltage query, millivolts */
        Serial.print('{'); Serial.print(CommandSerialNumber); Serial.print('_');
        Serial.print((int)(VoltageData_V * 1000)); Serial.print('}');
        break;
      case 25: /* mod: telemetry stream period in ms (0 = off) */
        StreamPeriod_ms = doc["D1"];
        ActiveStream = doc["D2"] == 1;
        if (ActiveStream && StreamPeriod_ms > 0 && StreamPeriod_ms < 100) StreamPeriod_ms = 100;
        Stream_time = _millis();
#if _is_print
        Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
