...
void DeviceDriverSet_Motor::DeviceDriverSet_Motor_Init(void)
{
  ...
  pinMode(PIN_Motor_STBY, OUTPUT);   // mod: wake the TB6612 driver
                                                          ...
                                                          )                                     //电机控制
{
  ...
  if (controlED == control_enable) //使能允许？
  {
    { //A...Right
      ...
      {
      case direction_just:
        digitalWrite(PIN_Motor_AIN_1, MOTOR_A_FWD_LEVEL);    // mod: polarity per shield (header)
        analogWrite(PIN_Motor_PWMA, speed_A);
        break;
      case direction_back:

        digitalWrite(PIN_Motor_AIN_1, !MOTOR_A_FWD_LEVEL);
        analogWrite(PIN_Motor_PWMA, speed_A);
        break;
      case direction_void:
...
void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_Get(uint16_t *ULTRASONIC_Get /*out*/)
{
  ...
  digitalWrite(TRIG_PIN, LOW);
  tempda_x = ((unsigned int)pulseIn(ECHO_PIN, HIGH, 30000UL) / 58); // mod: 30 ms timeout ~ 5 m, no 1 s stall on a miss
  // *ULTRASONIC_Get = tempda_x;

  if (tempda_x == 0 || tempda_x > 400) // mod: cap raised 150 -> 400 cm; 0 (timeout) also reports as no echo
  {
    *ULTRASONIC_Get = 400;
  }
  else
...
ActivePan activePan;
// mod: pan independently of wheel mode; loop continues integrating gyro and watchdog.
void DeviceDriverSet_Servo::panStart(int angle)
{
  myservo.attach(PIN_Servo_z);
  myservo.write(activePan.move(angle, _millis()));
}
void DeviceDriverSet_Servo::panTick()
{
  if (activePan.attached && !activePan.busy(_millis())) {
    myservo.detach();
    activePan.attached = false;
  }
}
...
void DeviceDriverSet_Servo::DeviceDriverSet_Servo_controls(uint8_t Servo, unsigned int Position_angle)
{
  ...
  {
    if (Position_angle < 10) // mod: angle is now plain degrees, 10..170
    {
      Position_angle = 10;
    }
    if (Position_angle > 170)
    {
      Position_angle = 170;
    }
    myservo.attach(PIN_Servo_z);
    myservo.write(Position_angle);
    _delay(200); // mod: was 500; the SG90 settles a 90-degree move in well under 200 ms
  }
