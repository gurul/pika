...
// mod: this car's shield is the 2020 TB6612 board (its original firmware printed MPU6050_chip_id).
// The V0_20210104 source targets the DRV8835 board, which has no standby pin and swaps the
// direction pins; with that map the motors never move here. Set SHIELD_DRV8835 to build for that board.
#ifdef SHIELD_DRV8835
#define PIN_Motor_PWMA 5
#define PIN_Motor_PWMB 6
#define PIN_Motor_BIN_1 7
#define PIN_Motor_AIN_1 8
#define PIN_Motor_STBY 3   // unused on that board, kept so Motor_Init compiles
#define MOTOR_A_FWD_LEVEL LOW
#else
...
// measured with the gyro 2026-09-22: on this shield AIN_1 LOW drives group A backward,
// so forward needs HIGH (group B keeps HIGH = forward as in the DRV8835 source)
#define MOTOR_A_FWD_LEVEL HIGH
#endif
