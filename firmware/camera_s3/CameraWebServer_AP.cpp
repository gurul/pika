...
// Setup LED FLash if LED pin is defined in camera_pins.h
  ...
  // Home router (ESP32 is 2.4 GHz only). ELEGOO soft-AP stays available for direct link.
  // Credentials: copy secrets.h.example to secrets.h (see README in this folder).
  ...
  const unsigned long staTimeoutMs = 60000;
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("ELEGOO AP still available.");
  }
